...
{
    public partial class loginmedecin : Form
    {
        public loginmedecin()
        {
            InitializeComponent();
        ...
        private void buttonlogin_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand command = new MySqlCommand("SELECT * FROM `doctorinfo` WHERE `uname`=@username and `pass`=@pass", db.getConnection());
            ...
            if (table.Rows.Count > 0)
            {
                ...
                this.Hide();
                menuprincipalmedecin patients = new menuprincipalmedecin();
                patients.Show();
            }
            else
            {
